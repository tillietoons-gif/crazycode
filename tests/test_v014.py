...
from pycode.config import _flatten, parse_toml, project_config_path
from pycode.wizard import (dump_config, needs_wizard, reset_wizard, run_wizard,
                           wizard_done, wizard_marker_path, write_config_for)
...
class TestNeedsWizard(unittest.TestCase):
    ...
    def test_force_and_no_wizard_flags(self):
        self.assertFalse(
            needs_wizard(False, interactive=True, no_wizard=True, home=self.home)
        )
        self.assertTrue(
            needs_wizard(False, interactive=False, force=True, home=self.home)
        )
...
class TestRunWizard(unittest.TestCase):
    ...
    def test_happy_path_openai(self):
        ...
        out, lines = collector()
        result = run_wizard(
            input_fn=fake, output_fn=out, root=self.root, home=self.home
        )
...
class TestConfigApiKey(unittest.TestCase):
    def test_provider_api_key_survives_flatten(self):
        flat = _flatten(
            {"provider": {"api_key": "sk-file", "model": "m"}, "theme": "mono"}
        )
