...
from pycode.config import coalesce, find_config_files, load_config
from pycode.mcp import MCPRegistry
from pycode.onboarding import (has_any_credential, mark_onboarded,
                               onboarding_message, should_show_onboarding)
...
from pycode.tui import (bold, build_dashboard_state, c, dim, print_banner,
                        render_linear_dashboard, result_badge)
...
def _confirm_prompt(tool: str, args: dict) -> bool:
    ...
    if tool == "bash":
        ...
        summary = (
            f"edit {args.get('path','?')} (old={args.get('old_string','')[:60]!r})"
        )
    print(
        f"\n{bold(c('red', '[CONFIRM]'))} Destructive action: {summary}",
        file=sys.stderr,
    )
    try:
...
def _run_loop_with_abort(
    agent: Agent, goal: str, loop_max: int, use_tui: bool = False
) -> dict:
    ...
    for path in specs:
        ...
        for entry in entries:
            ...
            if not name or not command:
                print(
                    f"{c('yellow','warn:')} MCP spec missing name/command: {entry}",
                    file=sys.stderr,
                )
                continue
...
def main() -> None:
    ...
    parser.add_argument(
        "prompt",
        nargs="*",
        help="Optional initial prompt (if omitted, interactive mode)",
    )
    ...
    parser.add_argument(
        "--model",
        help="Model name, or a preset (openai/anthropic/ollama/venice/openrouter)",
    )
    parser.add_argument(
        "--preset",
    ...
    parser.add_argument(
        "--system-prompt-file", help="Path to extra system instructions"
    )
    parser.add_argument(
        "--max-iterations",
        type=int,
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress stderr progress output"
    )
    parser.add_argument(
        "--non-interactive", action="store_true", help="Run prompt then exit (no REPL)"
    )
    parser.add_argument(
        "--auto-approve",
        action="store_true",
    ...
    parser.add_argument(
        "--resume", metavar="FILE", help="Resume from a saved JSONL session file"
    )
    parser.add_argument(
        "--project-root",
        default=os.getcwd(),
    ...
    parser.add_argument(
        "--permissions", metavar="FILE", help="Path to a permissions.toml policy file"
    )
    parser.add_argument(
        "--yolo",
        action="store_true",
    ...
    parser.add_argument(
        "--cost", action="store_true", help="Show token/cost accounting on every turn"
    )
    parser.add_argument(
        "--no-cost", action="store_true", help="Disable cost accounting entirely"
    )
    parser.add_argument(
        "--plain",
        action="store_true",
    ...
    parser.add_argument(
        "--no-wizard", action="store_true", help="Never auto-run the setup wizard"
    )
    parser.add_argument(
        "--no-tab-complete",
        action="store_true",
    ...
    args.auto_approve = (
        args.auto_approve
        or bool(file_cfg.get("auto_approve"))
        or bool(file_cfg.get("always_yes"))
        or args.always_yes
    )
    ...
    context_budget = coalesce(
        args.context_budget, file_cfg.get("context_budget"), 60000
    )
    ...
    if plugin_summary["loaded"] and not args.quiet:
        print(
            dim(
                f"  plugins: {len(plugin_summary['loaded'])} user tool(s): "
                + ", ".join(plugin_summary["loaded"])
            ),
            file=sys.stderr,
        )
    for err in plugin_summary["errors"]:
        print(
            f"{c('yellow','warn:')} plugin {os.path.basename(err['path'])}: {err['error']}",
            file=sys.stderr,
        )
    user_commands = plugins.load_user_commands(args.project_root)
    if user_commands and not args.quiet:
        print(
            dim("  commands: " + ", ".join("/" + n for n in sorted(user_commands))),
            file=sys.stderr,
        )
    ...
    cfg = {
        ...
        "temperature": coalesce(
            args.temperature, env_cfg.get("temperature"), file_cfg.get("temperature")
        ),
        "max_tokens": coalesce(
            args.max_tokens, env_cfg.get("max_tokens"), file_cfg.get("max_tokens")
        ),
        "system_prompt_extra": args.system_prompt_file
        and open(args.system_prompt_file).read()
    ...
    if args.mcp:
        ...
        if not args.quiet:
            ...
            print(
                dim(f"  mcp: {len(registry.servers)} server(s), {n_tools} tool(s)"),
                file=sys.stderr,
            )
    ...
    if args.failover:
        ...
        if not provs:
            print(
                f"{c('yellow','warn:')} no providers in {args.failover}",
                file=sys.stderr,
            )
        else:
            ...
            if not args.quiet:
                print(
                    dim(
                        f"  failover: {len(provs)} provider(s): "
    ...
    if not cfg.get("api_key"):
        ...
        if should_show_onboarding(force=args.force_onboard):
            print(
                c("yellow", "  \U0001f680 first run - no LLM key detected"),
                file=sys.stderr,
            )
            ...
            print(
                "  Set PYCODE_API_KEY or --api-key, or use a preset with its env key.",
                file=sys.stderr,
            )

    if args.use_tui:
        project_name = (
            os.path.basename(os.path.abspath(args.project_root)) or "workspace"
        )
        ...
        print(
            render_linear_dashboard(
                ...
                nav=dashboard_state.get(
                    "nav", [("Inbox", 0), ("Active", 0), ("Review", 0), ("Done", 0)]
                ),
                cards=dashboard_state.get("cards", []),
                selected=dashboard_state.get("selected")
                or (
                    dashboard_state.get("cards", [{}])[0]
                    if dashboard_state.get("cards")
                    else {}
                ),
        ...
        print(
            dim(
                "  interactive mode · type /help for commands · /theme <name> to switch palettes"
            ),
            file=sys.stderr,
            flush=True,
        )
    ...
    if args.agent_loop:
        summary = _run_loop_with_abort(
            agent, args.agent_loop, loop_max, use_tui=args.use_tui
        )
        print(
            bold(
                f"  auto-fix loop: {summary['attempts']} attempt(s), "
    ...
    if prompt_parts:
        interactive_review = (
            args.dry_run and not args.non_interactive and sys.stdin.isatty()
        )
    ...
    while True:
        ...
        if user_input.startswith("/context"):
            ...
            if args.use_tui:
                print_context(
                    usage["used"], usage["budget"], usage["messages"], use_color=True
                )
            else:
                print(
        ...
        if user_input.startswith("/loop"):
            ...
            if not goal:
                print(
                    f"usage: /loop <goal>   (auto-fix cycle, up to {loop_max} attempts)",
                    file=sys.stderr,
                )
                continue
            print(
                dim(f"  auto-fix loop ({loop_max} max attempts): {goal}"),
                file=sys.stderr,
            )
            ...
            print(
                bold(
                    f"  loop finished: {summary['attempts']} attempt(s), "
                    f"{'achieved' if summary.get('achieved') else 'not confirmed achieved'}"
                ),
                file=sys.stderr,
            )
        ...
        if user_input.startswith("/plan"):
            ...
            print(
                dim(f"  executed {total}/{len(summary.get('steps', []))} step(s)"),
                file=sys.stderr,
            )
        ...
        if user_input.startswith("/map"):
            ...
            st = idx.stats()
            print(
                dim(f"  ({st['files']} files, {st['symbols']} symbols)"),
                file=sys.stderr,
            )
            continue
        if user_input.startswith("/symbols"):
            ...
            for r in results[:25]:
                print(
                    f"  {r['kind']:<8} {r['name']:<24} {os.path.relpath(r['path'], args.project_root)}:{r['line']}",
                    file=sys.stderr,
                )
            continue
        ...
        if user_input.startswith("/rewind"):
            ...
            if target >= len(agent.rewinder.checkpoints):
                print(
                    f"checkpoint {target} out of range (0-{len(agent.rewinder.checkpoints)-1})",
                    file=sys.stderr,
                )
                continue
            ...
            continue
        if user_input.startswith("/new-context") or user_input.startswith(
            "/context-file"
        ):
            from pycode.scaffold import exists, generate

            target = "CLAUDE.md"
            try:
                path = generate(name=target, root=args.project_root, overwrite=True)
                print(
                    f"Wrote {path} (edit it to add your project's conventions)",
                    file=sys.stderr,
                )
            except Exception as e:
        ...
        if user_input.startswith("/resume"):
            ...
            try:
                ...
                agent.messages = [agent.messages[0]] + msgs
                print(
                    f"Resumed session: {target} ({len(msgs)} messages)", file=sys.stderr
                )
            except Exception as e:
            ...
            if args.use_tui and sys.stdin.isatty():
                ...
                if chosen:
                    ...
                    try:
                        ...
                        agent.messages = [agent.messages[0]] + msgs
                        print(
                            f"Resumed session: {chosen.short_name} ({len(msgs)} messages)",
                            file=sys.stderr,
                        )
                    except Exception as e:
                        print(
                            f"Error resuming {chosen.short_name}: {e}", file=sys.stderr
                        )
            else:
        ...
        if user_input.startswith("/export"):
            ...
            try:
                ...
                print(
                    f"Exported session to {path} ({len(agent.messages)} messages)",
                    file=sys.stderr,
                )
            except Exception as e:
        ...
        if user_input.startswith("/rewind"):
            ...
            try:
                target = int(idx)
                note = (
                    input("  rewind note (optional, Enter to skip): ").strip() or None
                )
                count = agent.rewind(target, note=note)
                print(
                    f"Rewound to checkpoint #{target} ({count} messages).",
                    file=sys.stderr,
                )
            except ValueError:
        ...
        if user_input.startswith("/branch"):
            ...
            if len(parts) < 2:
                print(
                    "usage: /branch <checkpoint-index> <new instruction>",
                    file=sys.stderr,
                )
                continue
            try:
                ...
                print(
                    f"Branch created from #{target} ({count} messages).",
                    file=sys.stderr,
                )
            except (ValueError, IndexError):
                print(
                    "invalid /branch syntax: /branch <index> <instruction>",
                    file=sys.stderr,
                )
            continue
        ...
        if cmd_name in user_commands:
            ...
            user_input = plugins.apply_command(
                user_commands[cmd_name]["template"], args_str
            )
        ...
        if args.cost and agent.cost_tracker is not None:
            ...
            print(
                dim(
                    f"  [cost] last turn ~${rep['last_turn_cost_usd']:.4f} | "
                    f"session ~${rep['session_cost_usd']:.4f}"
                ),
                file=sys.stderr,
            )
